...
int AVL::height(node* temp) {      //To calculate height of subtree
    int lh, rh;
    if(temp == NULL)
        return 0;
...
int AVL::BF(node* temp) {          //To calculate Balance factor i.e left height - right height
    if(temp == NULL)
        return 0;
...
node* AVL::RR(node *T)  //RR rotation
{
    T = RotateLeft(T);
    return T;
}
node* AVL::LL(node *T) //LL notation
{
    T=RotateRight(T);
    return T;
}
node* AVL::LR(node *T) //LR rotation 
{
    T->left = RotateLeft(T->left);
    T = RotateRight(T);
    return T;
}
node* AVL::RL(node *T) //LR rotation
{
    T->right = RotateRight(T->right);
    T = RotateLeft(T);
    return T;
}
node* AVL::insert(node* temp, string str_w, string str_m) // Inserting node into the tree
{
    if(temp == NULL)
    {
        temp = new node;
